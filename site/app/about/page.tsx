    ...
    <>
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          <Image
            ...
            priority
          />
          <div className="absolute inset-0 bg-gradient-to-b from-cream-100/80 via-cream-100/55 to-cream-100" />
        </div>
        <div className="relative mx-auto flex min-h-[60vh] max-w-4xl flex-col items-center justify-center px-4 py-20 text-center md:px-6">
          <p className="text-sm font-semibold uppercase tracking-wider text-teal-600">About {BIZ.name}</p>
      ...
      <section className="py-16">
        <div className="mx-auto max-w-7xl px-4 md:px-6">
          <div className="mx-auto max-w-3xl text-center">
            <p className="text-sm font-semibold uppercase tracking-wider text-teal-600">Our approach</p>
            <h2 className="mt-2 font-display text-3xl font-bold tracking-tight md:text-5xl">
              Measurement and code compliance come first.
            </h2>
            <p className="mt-4 text-cream-600">
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {APPROACH.map(({ Icon, label, value, body }) => (
              <article key={label} className="rounded-sm border border-cream-300 bg-cream-50/50 p-5">
                <Icon className="h-5 w-5 text-teal-600" />
                <div className="mt-3 text-sm text-cream-500">{label}</div>
                <h3 className="font-display font-bold text-white">{value}</h3>
                <p className="mt-2 text-sm text-cream-600">{body}</p>
              </article>
            ))}
          </div>
        </div>
          ...
          <div className="mx-auto max-w-7xl px-4 md:px-6">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {inspiration.map((photo) => (
                <div key={photo.slug} className="overflow-hidden rounded-sm border border-cream-300">
                  <Image
                    src={photo.src}
                    alt={photo.label}
                    width={1600}
      ...
      <section className="py-16">
        <div className="mx-auto max-w-4xl px-4 md:px-6">
          <div className="rounded-3xl border border-teal-500/30 bg-gradient-to-br from-teal-500/10 to-cream-200/40 p-8 text-center">
            ...
            <div className="mt-5 flex flex-wrap items-center justify-center gap-3 text-sm">
              <span className="inline-flex items-center gap-1.5 rounded-full border border-cream-300 bg-cream-50/70 px-3 py-1.5">
                <Phone className="h-4 w-4 text-teal-600" /> {BIZ.phone}
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-full border border-cream-300 bg-cream-50/70 px-3 py-1.5">
                <MapPin className="h-4 w-4 text-teal-600" /> {BIZ.address.full}
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-full border border-cream-300 bg-cream-50/70 px-3 py-1.5">
                <Clock className="h-4 w-4 text-teal-600" /> Mon–Fri 7–6 · Sat 8–2
              </span>
            </div>
