  ...
  return (
    <>
      <section className="relative overflow-hidden border-b border-cream-300 bg-cream-100">
        ...
        />
        <div className="absolute inset-0 z-0 bg-gradient-to-t from-cream-100 via-cream-100/80 to-cream-100/40" />
        <div className="relative z-10 mx-auto max-w-7xl px-4 py-20 md:px-6 md:py-28">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="inline-flex items-center gap-2 rounded-full border border-cream-300 bg-cream-100/60 px-3 py-1.5 text-xs font-semibold text-cream-700 backdrop-blur">
              <MapPin className="h-3.5 w-3.5 text-teal-600" /> Manhattan &amp; Brooklyn
            </span>
          </div>
          <div className="mt-5 flex items-center gap-3">
            <Icon className="h-7 w-7 text-teal-600" />
            <p className="font-display text-sm font-semibold uppercase tracking-[0.2em] text-teal-600">
              {s.shortName}
            </p>
          </div>
          ...
          <p className="mt-4 max-w-2xl text-lg text-cream-700">{s.tagline}</p>
          <div className="mt-7">
            <ContactCTA size="lg" />
          </div>
        </div>
      ...
      <section className="border-b border-cream-300 bg-cream-100 py-12 md:py-16">
      ...
      <section className="py-16">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 md:grid-cols-3 md:px-6">
          <Reveal className="md:col-span-2">
            ...
            <p className="mt-4 text-cream-700">{s.description}</p>
            <RevealStagger className="mt-6 grid gap-3 sm:grid-cols-2" stagger={0.06}>
              {s.bullets.map((b) => (
                <RevealItem key={b}>
                  <div className="flex h-full items-start gap-3 rounded-sm border border-cream-300 bg-cream-50/50 p-4 transition hover:-translate-y-0.5 hover:border-teal-500/40">
                    <span className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full bg-teal-500/15 text-teal-600">
                      <Check className="h-3.5 w-3.5" />
                    </span>
                    <span className="text-sm text-cream-800">{b}</span>
                  </div>
                </RevealItem>
              ))}
            </RevealStagger>
          ...
          <Reveal delay={0.1} variant="tilt" className="space-y-4">
            ...
            <div className="rounded-3xl border border-cream-300 bg-cream-50/50 p-5">
              <h3 className="font-display text-lg font-bold">Request this service</h3>
              <p className="mt-1 text-sm text-cream-600">Share your property, opening, and preferred timing.</p>
              <div className="mt-4">
                <ContactCTA size="md" />
              </div>
            </div>
      ...
      <section className="border-t border-cream-300 py-12">
        <div className="mx-auto max-w-3xl px-4 text-center text-sm text-cream-500 md:px-6">
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-6">
          <Reveal>
            <div className="mb-6 flex items-end justify-between gap-4">
              <div>
                <p className="text-sm font-semibold uppercase tracking-wider text-teal-600">Service area</p>
                <h2 className="mt-2 font-display text-2xl font-bold md:text-3xl">
                  {s.shortName} service across NYC
                </h2>
                <p className="mt-2 max-w-2xl text-cream-600">
                  Door service across Manhattan &amp; Brooklyn during posted business hours.
                </p>
              </div>
              <Link href="/service-areas" className="hidden text-sm font-semibold text-teal-600 hover:text-teal-600 md:inline-flex">
                All service areas →
              </Link>
            </div>
          </Reveal>
      ...
      <section className="border-t border-cream-300 py-16">
        <div className="mx-auto max-w-3xl space-y-5 px-4 text-sm text-cream-700 md:px-6 md:text-base">
          ...
          <p>
            ...
            <a href="/service-areas" className="text-teal-600 underline-offset-2 hover:underline">coverage map</a>.
      ...
      <section className="border-t border-cream-300 bg-aurora py-16 text-center">
        <div className="mx-auto max-w-3xl px-4 md:px-6">
          <Reveal>
            ...
            <p className="mt-3 text-cream-700">Tell {BIZ.name} about the opening and door type you have in mind.</p>
